...
We compare four different models: the one used by \citet{diluvi2018}, which contains modified priors but samples from the posterior in \citet{mendoza-nieto2016}; that same model but now sampling from the correct posterior via MCMC; the model proposed in Section \ref{sec:model} with the priors discussed therein; and the same model but assuming a trivial correlation structure, i.e., $P = I_J$. In this last model, the proportions of votes are forced to live in the probabilistic simplex but the correlation structure assumes a simple form, which results in faster sampling.
